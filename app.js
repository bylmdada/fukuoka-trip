            ...
            ${taoyuanParkingData.lots.map(lot => `
                <div class="parking-card ${lot.recommended ? 'recommended' : ''}">
                    <div class="parking-header">
                        <h4>${lot.name} ${lot.recommended ? '<span class="parking-rec">⭐推薦</span>' : ''}</h4>
                    </div>
                    <div class="parking-details">
                        <p class="parking-addr">📍 ${lot.addr}</p>
                        <p class="parking-shuttle">🚌 ${lot.shuttle}</p>
                        <p class="parking-points">🌳 ${lot.points}</p>
                        <p class="parking-note">💡 ${lot.note}</p>
                    </div>
...
function addDayTripsSection() {
    ...
    const dayTrips = [
        ...
        },
        {
            ...
            season: '全年（冬季10:00開園）',
            color: '#795548',
            transport: '天神→高速巴士約2.5h→別府→龜之井巴士41號紈50分',
            highlights: ['叢林巴士', '餵食獅子', '餵食大象', '小動物互動', '別府溫泉', '包車≈巴士價'],
            desc: '日本最大級野生動物園！必搭「叢林巴士」餵食獅子、大象。9人團純交通：包車約¥8,900／人，大眾運輸約¥7,980／人，差約¥1,000。親子團推薦包車（不用搬娃娃車追巴士）！',
            mapUrl: 'https://www.google.com/maps/search/?api=1&query=九州自然動物公園アフリカンサファリ',
            tips: '冬季10:00開園、最後入園15:00・叢林巴士需開園前30分排隊',
            price: '交通約¥8-9K／人｜門票¥2,600｜叢林巴士¥1,300'
        }
    ];
        ...
        <div class="transport-summary">
            ...
            <div class="transport-items">
                ...
                <div class="transport-item">
                    ...
                    <span class="route-detail">天神→高速巴士約2.5h→別府→巴士紈50分｜包車≈¥8.9K vs 巴士≈¥8K／人</span>
                </div>
            </div>
        </div>
    `;
